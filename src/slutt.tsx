...
function ParticipantDropdown({ stage, selected, onSelect }: { stage: string; selected: string; onSelect: (id: string) => void }) {
  ...
  if (selectedObj) {
    if (statusForStage[selectedObj.id] === 'DNS') {
      ...
      selectedClass = 'px-4 py-2 rounded cursor-pointer border-2 border-red-600 font-bold';
      selectedStyle = { backgroundColor: '#dc2626', color: '#fff', fontWeight: 'bold' };
    } else if (timesForStage[selectedObj.id]?.end) {
      selectedSuffix = ` ${timesForStage[selectedObj.id].end}`;
    } else {
      selectedSuffix = '';
    }
  }
          ...
          <div className="absolute z-50 w-full border-2 border-blue-500 rounded-md bg-blue-50 shadow-2xl mt-2 max-h-96 overflow-y-auto">
            ...
            {participants.map((p: any, idx: number) => {
              ...
              if (statusForStage[p.id] === 'DNS') {
                ...
                suffix = ' (DNF)';
                rowClass += ' bg-red-600 text-white font-bold';
              } else if (timesForStage[p.id]?.end) {
                suffix = ` ${timesForStage[p.id].end}`;
              } else {
                suffix = '';
              }
              if (selected === p.id) rowClass += ' ring-2 ring-purple-700';
              return (
                <div
                  key={p.id}
                  className={rowClass}
...
function SluttApp() {
  // Statisk flyt for sluttregistrering
  // 1. Velg etappe
  // 2. Velg deltaker
  // 3. Registrer sluttid
  ...
  return (
    <>
      ...
      <div className="min-h-screen flex flex-col items-center justify-center p-4 pt-20">
        <h1 className="text-2xl font-bold mb-6">Registrer Sluttid</h1>
        {!stage ? (
          ...
          <div className="w-full max-w-md mx-auto">
            <TimeRegistrationStatic stage={stage} participant={participant} type="end" />
          </div>
        )}
      </div>
    </>
      ...
      <div className="flex flex-col gap-3">
        {stages.map((stage: string) => (
          <button
            ...
            className="bg-gray-200 hover:bg-purple-700 hover:text-white py-3 rounded-md font-medium"
          >
            {stage}
          </button>
        ))}
...
function ParticipantSelectorStatic({ stage, onSelect }: { stage: string; onSelect: (id: string) => void }) {
  ...
  React.useEffect(() => {
    ...
    participants.forEach((p: any) => {
      ...
      else if (timesForStage[p.id]?.end) data[p.id] = timesForStage[p.id].end;
      else data[p.id] = '';
    });
    ...
    <div className="text-center">
      ...
      <div className="flex flex-col sm:flex-row flex-wrap gap-2 justify-center mb-4">
        <button
          ...
          className="bg-purple-700 text-white px-6 py-4 rounded-md w-full sm:w-auto text-base sm:text-lg"
      ...
      {confirmed && (
        <div className="p-4 bg-purple-100 border border-purple-400 rounded-md mt-4">
